...
import {
  UPDATE_PIER,
  UPDATE_PIERVariables as UPDATE_PIER_VARS,
} from './__generated__/UPDATE_PIER';
...
import {
  DELETE_PIER,
  DELETE_PIERVariables as DELETE_PIER_VARS,
} from './__generated__/DELETE_PIER';

interface Props
  extends Omit<FormProps<Pier>, 'initialValues' | 'isSubmitting'> {
  pierId: string;
}

const PierEditForm: React.FC<Props> = ({
  pierId,
  onCancel,
  onSubmit,
  onDelete,
  ...
  const { loading, error, data } = useQuery<PIER_AND_BOAT_TYPES>(
  ...
  return (
    <PierForm
      ...
      onDelete={values =>
        deletePier({ variables: { input: { id: pierId } } }).then(() =>
          onDelete?.(values)
        )
      }
